...
from tests.data import (
    ...
    invalid_seckey_length, invalid_pubkey_length, invalid_signature_length
)
...
class TestPysecp256k1Schnorrsig(unittest.TestCase):
    ...
    def test_schnorrsig_sign_invalid_input_type_aux_rand32(self):
        ...
        for invalid_msg in invalid_seckey_length:
            with self.assertRaises(ValueError):
                schnorrsig_sign(keypair, os.urandom(32), aux_rand32=invalid_msg)

        for invalid_type in not_bytes[1:]:  # omit None as it is optional
            with self.assertRaises(ValueError):
                schnorrsig_sign(keypair, os.urandom(32), aux_rand32=invalid_type)

    def test_schnorrsig_sign_custom_invalid_input_type_keypair(self):
        for invalid_keypair in invalid_keypair_length:
            with self.assertRaises(ValueError):
                schnorrsig_sign_custom(invalid_keypair, os.urandom(32))

        for invalid_type in not_c_char_array:
            with self.assertRaises(ValueError):
                schnorrsig_sign_custom(invalid_type, os.urandom(32))
    ...
    def test_schnorrsig_verify_invalid_input_type_compact_sig(self):
        ...
        for invalid_sig in invalid_signature_length:
            with self.assertRaises(ValueError):
                schnorrsig_verify(invalid_sig, os.urandom(32), xonly_pubkey)

        for invalid_type in not_bytes:
            with self.assertRaises(ValueError):
                schnorrsig_verify(invalid_type, os.urandom(32), xonly_pubkey)
    ...
    def test_schnorrsig_verify_invalid_input_type_msg(self):
        ...
        for invalid_type in not_bytes:
            with self.assertRaises(ValueError):
                schnorrsig_verify(64 * b"\x00", invalid_type, xonly_pubkey)

    def test_schnorrsig_verify_invalid_input_type_xonly_pubkey(self):
        for invalid_xonly in invalid_pubkey_length:
            with self.assertRaises(ValueError):
                schnorrsig_verify(64 * b"\x00", os.urandom(32), invalid_xonly)

        for invalid_type in not_c_char_array:
            with self.assertRaises(ValueError):
                schnorrsig_verify(64 * b"\x00", os.urandom(32), invalid_type)
